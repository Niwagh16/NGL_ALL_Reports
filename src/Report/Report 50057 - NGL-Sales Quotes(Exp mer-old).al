report 50057 "NGL-Sales Quotes(Exp mer-old)"
{
    // version KAR1.0

    DefaultLayout = RDLC;
    RDLCLayout = 'src\Report Layout\NGL-Sales Quotes(Exp mer-old).rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            DataItemTableView = SORTING("Document Type", "No.")
                                WHERE("Document Type" = CONST(Quote)); //PCPL/NSW/MIG 09Aug22
            ...
            dataitem("<Sales Line>"; "Sales Line")
            {
                ...
                column(Tax_Amount; 0)//"Tax Amount")
                {
                }
                column(Excise_Amount; 0)// "Excise Amount")
                {
                }
                column(Service_tax_Amount; 0)//"Service Tax Amount")
                {
                }
                column(Service_tax_excess_Amount; 0)//"Service Tax eCess Amount")
                {
                }
            ...
            begin
                ...
                //port name instead of port code;

                // RecPort.RESET;   PCPL/NSW/MIG 09Aug22
