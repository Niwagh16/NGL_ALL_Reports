report 50092 "Order Inventory"
{
    // version NAVW17.10

    DefaultLayout = RDLC;
    RDLCLayout = 'src\Report Layout\Order Inventory.rdl';
    CaptionML = ENU = 'Order Inventory',
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                column(QtyValue1; QtyValue[1])
                {
                }
                column(QtyValue2; QtyValue[2])
                {
                }
                column(QtyValue3; QtyValue[3])
                {
                }
                ...
                begin
                    ...
                    ProdQty := 0;
                    CLEAR(QtyValue);
                    IF Customer_l.GET("Sales Line"."Sell-to Customer No.") THEN
                        CustomerName := Customer_l.Name;
                    IF Item_l.GET("Sales Line"."No.") THEN BEGIN
                        Item_l.CALCFIELDS(Inventory);
                        Item_l.CALCFIELDS("Qty. on Prod. Order");
                        IF ItemNo <> Item_l."No." THEN BEGIN
                            InvQty := Item_l.Inventory;
                            ProdQty := Item_l."Qty. on Prod. Order";
                            ItemNo := Item_l."No.";
                            NewDate[1] := TODAY;
                            StartDate[1] := CALCDATE('CM-1M+1D', NewDate[1]);
                            EndDate[1] := CALCDATE('CM', NewDate[1]);
                            NewDate[2] := CALCDATE('1M', StartDate[1]);
                            StartDate[2] := CALCDATE('-CM', NewDate[2]);
                            EndDate[2] := CALCDATE('CM', StartDate[2]);
                            NewDate[3] := CALCDATE('1M', StartDate[2]);
                            StartDate[3] := CALCDATE('-CM', NewDate[3]);
                            EndDate[3] := CALCDATE('CM', StartDate[3]);

                            FOR i := 1 TO 3 DO BEGIN
                                ProdOrder.RESET;
                                ProdOrder.SETRANGE(Status, ProdOrder.Status::"Firm Planned");
                                ProdOrder.SETRANGE("Source No.", "Sales Line"."No.");
                                IF ProdOrder.FINDSET THEN
                                    REPEAT
                                        IF (ProdOrder."Due Date" >= StartDate[i]) AND (ProdOrder."Due Date" <= EndDate[i]) THEN
                                            QtyValue[i] := QtyValue[i] + ProdOrder.Quantity;
                    ...
                    ///QtyValue [3]  := QtyValue[2] - QtyValue[1];
                    //PCPL0039
    ...
    var
        ...
        NewDate: array[3] of Date;
        i: Integer;
        QtyValue: array[3] of Decimal;
    ...
    begin
        ...
        ExcelBuf.AddColumn(''/*"Sales Line"."Identification Mark"*/, FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        //PCPL-0041-END-17012020
