report 50083 "Tax Invoice - NGL"
{
    // version RSPL/NGL/REPORT-2

    DefaultLayout = RDLC;
    RDLCLayout = 'src\Report Layout\Tax Invoice - NGL.rdl';
    ...
    dataset
    {
        dataitem(Pageloop; Integer)
        {
            ...
            dataitem("Sales Invoice Header"; "Sales Invoice Header")
            {
                ...
                column(CIN_No; 0) //Rec_Comp."Company Registration  No.") //PCPL-Deepak
                {
                }
                ...
                column(Excode; '') //recExciseProdSetup.Code) //PCPL-Deepak
                {
                }
                ...
                dataitem(Integer; Integer)
                {
                    ...
                    column(Buyer_Ref_No; "Sales Invoice Header"."Your Reference")
                    {
                    }
                    column(LocAdd; LocAdd)
                    {
                    }
                    column(LocTel; LocTel)
                    {
                    }
                    column(LocFax; LocFax)
                    {
                    }
                    column(LocCst; LocCst)
                    {
                    }
                    column(LocVat; LocVat)
                    {
                    }
                    column(txtshipcountry; txtShipToCustCountry)
                    {
                    }
                    column(locDrugLic; locDrugLic)
                    {
                    }
                    column(Buyer_Ref_no2; 0) //"Sales Invoice Header"."Buyer Reference No.1") //PCPL-Deepak
                    {
                    }
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(ADE_Vat; 0) //"Sales Invoice Line"."ADE Amount") //PCPL-Deepak
                        {
                        }
                        ...
                        column(SIL_Charges; '') //"Sales Invoice Line"."Charges To Customer") //PCPL-Deepak
                        {
                        }
                        column(Sub_Total; 0) //"Sales Invoice Line".Amount + "Sales Invoice Line"."Excise Amount" + "Sales Invoice Line"."Tax Amount" + "Sales Invoice Line"."Charges To Customer") //PCPL-Deepak
                        {
                        }
                        column(TaxableAmt; 0) //"Sales Invoice Line".Amount + "Sales Invoice Line"."Excise Amount") //PCPL-Deepak
                        {
                        }
                        ...
                        begin
                            ...
                            IF sil1.FINDFIRST THEN
                                REPEAT
                                    Amt := Amt + sil1.Amount;
                                    ecess := ecess + 0; //sil1."eCess Amount"; //PCPL-Deepak
                                    tax := tax + 0; //sil1."Tax Amount"; //PCPL-Deepak

                                    Excise := Excise + 0; //sil1."Excise Amount"; //PCPL-Deepak
                                    she := she + 0; //sil1."SHE Cess Amount"; //PCPL-Deepak
                                UNTIL sil1.NEXT = 0;
                            ...
                            // IF ExcisePosSetup.FINDFIRST THEN BEGIN //PCPL-Deepak
                    ...
                    begin
                        ...
                        IF Rec_loc.FINDFIRST THEN BEGIN
                            ...
                            //added code by sanjay for locationwise report  27/07/2015
                            LocAdd := Rec_loc.Address + ' ' + Rec_loc."Address 2" + ' ' + Rec_loc.City + ' ' + Rec_loc."Post Code" + ', ';
                            LocTel := Rec_loc."Phone No.";
                            LocFax := Rec_loc."Fax No.";
                            LocCst := ''; //Rec_loc."C.S.T No."; //PCPL-Deepak
                            LocVat := ''; //Rec_loc."T.I.N. No."; //PCPL-Deepak
                            locDrugLic := ''; //Rec_loc."Drug license No." //PCPL-Deepak
                            //added code by sanjay end
                        END;
                        //Added by sanjay for country name   27/07/2015
                        recCountry.RESET;
                        recCountry.SETFILTER(recCountry.Code, Rec_loc."Country/Region Code");
                        IF recCountry.FIND('-') THEN
                            txtShipToCustCountry := recCountry.Name;
                        // Added by sanjay for country name
                        ...
                        IF Rec_Cust.FINDFIRST THEN BEGIN
                            ...
                            CECC := ''; //Rec_Cust."E.C.C. No."; //PCPL-Deepak
                        END;
                    end;
                }
...
CurrReport.SKIP;
    ...
    var
        ...
        CSTAmount: Decimal;
        //ExcisePosSetup: Record "Excise Posting Setup"; //PCPL-Deepak
        ADCper: Decimal;
        TaxableAmt: Decimal;
        cnt: Integer;
        TaxPer: Decimal;
        TaxType: Text[30];
        //recExciseProdSetup: Record "Excise Prod. Posting Group"; //PCPL-Deepak
        recItem: Record Item;
        CF: Decimal;
        bulddrag: Label 'ORGANIC CHEMICALS (BULK DRUGS)';
        Recloctest: Record "License Information";
        LocAdd: Text[150];
        LocTel: Text[14];
        LocFax: Text[14];
        LocCst: Text[12];
